...
.align 4 # 源码为2
1:	.byte 	0xDB,0xE4			# fsetpm for 287, ignored by 387  # 两个字节值是80287协处理器指令fsetpm的机器码，其作用是把80287设置为保护模式，387无需该指令
	ret
...
setup_idt:
	lea 	ignore_int,%edx			# lea操作取mem的32位16进制偏移地址，送到指定寄存器，ignore_int有效地址(偏移值) -> eax寄存器
	movl 	$0x00080000,%eax		# selector = 0x0008 = cs，将选择符0x0008置入eax的高16位
	movw 	%dx,%ax				# 将ignore_int偏移地址的低16位存入eax的低16位中.此时eax含有门描述符低2字节的值.
	movw 	$0x8E00,%dx			# 1000 1110 - P = 1，dpl=0，type = 1110 此时edx含有门描述符高4字节的值
...
gdt:
	...
	.quad	0x00c09a0000000fff	/* 16Mb */	# 0x08，内核代码段最大长度16MB base = 0x0000 0000
	.quad	0x00c0920000000fff	/* 16Mb */	# 0x10，内核数据段最大长度16MB base = 0x0000 0000
